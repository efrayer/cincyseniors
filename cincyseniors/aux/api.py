...
- Use markdown. Your response is rendered in a chat interface that displays markdown as formatted text.
- CRITICAL: Each bullet point MUST be on its own line. Put a blank line before the first bullet. Never run bullet items together on one line.
- For simple one-sentence answers, plain prose is fine — no headers needed.
- For answers with multiple points or topics, use bullet lists where each item is on a new line:

  - First point here
  - Second point here
  - Third point here

- Use a ## heading only when the response has clearly separate sections (e.g. multiple services or categories).
- Never use # (h1) headings — ## or ### only if needed.
- Keep bullet lists tight: one idea per bullet, no nested bullets unless truly necessary.
- Do not use raw markdown symbols as decoration (no --- dividers, no *** for emphasis without meaning).
- Do not end responses with a horizontal rule or a list of source names — sources are shown separately."""
...
async def cindy_chat(request: Request, req: CindyChatRequest):
    ...
    # "home care", "meals on wheels", or a proper noun pattern.
    query_lower = last_user.lower()
    directory_triggers = [
        "senior center", "senior service", "home care", "meals on wheels",
        "retirement", "assisted living", "nursing home", "day program",
        "resource center", "council on aging", "volunteer", "hospice",
    ]
...
def _validate_ingest_input(filename: str = "", title: str = "", text: str = "", url: str = ""):
    """Block path traversal, null bytes, and obviously malicious input."""
    import re
    if filename:
        if ".." in filename or "/" in filename or "\\" in filename or "\x00" in filename:
            raise HTTPException(status_code=400, detail="Invalid filename.")
        if not re.search(r'\.(pdf|txt|md)$', filename, re.IGNORECASE):
    ...
    if text and len(text) > 500_000:
        raise HTTPException(status_code=400, detail="Text content too large (max 500 KB).")
    if url:
        if not re.match(r'^https?://', url, re.IGNORECASE):
            raise HTTPException(status_code=400, detail="URL must start with http:// or https://.")
        if any(bad in url for bad in ["localhost", "127.0.0.1", "0.0.0.0", "169.254.", "::1"]):
            raise HTTPException(status_code=400, detail="Internal URLs are not permitted.")
...
async def cindy_ingest(files: list[UploadFile] = File(...), _: None = Depends(require_admin_key)):
    ...
    for f in files:
        _validate_ingest_input(filename=f.filename)
        contents = await f.read()
        ...
        # Filter out empty chunks (e.g. from scanned/image-only PDFs)
        ...
        if not chunks:
            results.append({
                ...
                "pages": len(docs),
                "chunks": 0,
                "warning": "No text could be extracted. The PDF may be scanned or image-based.",
            })
            continue
...
async def cindy_ingest_text(req: CindyIngestTextRequest, _: None = Depends(require_admin_key)):
    ...
    _validate_ingest_input(title=req.title, text=req.text)
...
async def cindy_ingest_url(req: CindyIngestUrlRequest, _: None = Depends(require_admin_key)):
    ...
    for url in req.urls:
        url = url.strip()
        _validate_ingest_input(url=url)
        if not url:
            continue
        try:
